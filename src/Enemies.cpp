...
void Enemies::moveEnemy(GridLoc plyLoc)
{
    GridLoc enmVal = this->getEnemyLoc();
    //cout<<enmVal.x<<" "<<plyLoc.y<<endl;

    if(moveDis<=0){
        if(enmVal.x < plyLoc.x)
        {
            right=true; down=left=up=false;
        }
        else if(enmVal.x > plyLoc.x)
        {
            left=true; down=up=right=false;
        }
        else if(enmVal.y < plyLoc.y)
        {
